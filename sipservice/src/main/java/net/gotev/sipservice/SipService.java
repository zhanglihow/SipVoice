...
public class SipService extends BackgroundService implements SipServiceConstants {
    ...
    public void onCreate() {
        ...
        enqueueJob(new Runnable() {
            @Override
            public void run() {
                TyLog.i("Creating SipService with priority: " + Thread.currentThread().getPriority());

                loadNativeLibraries();
                mSharedPreferencesHelper = SharedPreferencesHelper.getInstance(SipService.this).init(SipService.this);
                mBroadcastEmitter = new BroadcastEventEmitter(SipService.this);
                loadConfiguredAccounts();
//                addAllConfiguredAccounts();

                TyLog.i("SipService created!");
            }
        });
    }
        ...
        enqueueJob(new Runnable() {
            ...
            public void run() {
                ...
                if (mConfiguredAccounts.isEmpty() && mConfiguredGuestAccount == null) {
                    TyLog.d("No more configured accounts. Shutting down service");
                    stopSelf();
                }
            }
        });
    ...
    private void handleAcceptIncomingCall(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while accepting incoming call. AccountID: "
                         + accountID + ", CallID: " + callID);
        }
    }
    ...
    private void handleSetCallHold(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while setting hold. AccountID: "
                    + accountID + ", CallID: " + callID);
        }
    }
    ...
    private void handleToggleCallHold(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while toggling hold. AccountID: "
                    + accountID + ", CallID: " + callID);
        }
    }
    ...
    private void handleSetCallMute(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while setting mute. AccountID: "
                         + accountID + ", CallID: " + callID);
        }
    }
    ...
    private void handleToggleCallMute(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while toggling mute. AccountID: "
                    + accountID + ", CallID: " + callID);
        }
    }
    ...
    private void handleDeclineIncomingCall(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while declining incoming call. AccountID: "
                    + accountID + ", CallID: " + callID);
        }
    }
    ...
    private void handleHangUpCall(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while hanging up call:"+exc);
            notifyCallDisconnected(accountID, callID);
        }
    }
    ...
    private void handleHoldActiveCalls(Intent intent) {
        ...
        for (int callID : activeCallIDs) {
            try {
                ...
                TyLog.e("Error while holding call:"+exc);
            }
        }
    }
    ...
    private void handleTransferCall(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while transferring call to " + number, exc);
            notifyCallDisconnected(accountID, callID);
        }
    }
    ...
    private void handleMakeCall(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while making outgoing call:"+exc);
    ...
    private void handleRefreshRegistration(Intent intent) {
        ...
        if (!mActiveSipAccounts.isEmpty() && mActiveSipAccounts.containsKey(accountID)){
            try {
                ...
                TyLog.e("Error while refreshing registration");
                ex.printStackTrace();
            }
        } else {
            TyLog.d("account "+accountID+" not set");
        }
    }

    private void handleRestartSipStack() {
        TyLog.d("Restarting SIP stack");
        stopStack();
        addAllConfiguredAccounts();
    }

    private void handleResetAccounts() {
        TyLog.d("Removing all the configured accounts");
        ...
        while (iterator.hasNext()) {
            ...
            try {
                ...
                TyLog.e("Error while removing account " + data.getIdUri(), exc);
            }
        }
    ...
    private void handleRemoveAccount(Intent intent) {
        ...
        TyLog.d("Removing " + accountIDtoRemove);
        ...
        while (iterator.hasNext()) {
            ...
            if (data.getIdUri().equals(accountIDtoRemove)) {
                try {
                    ...
                    TyLog.e("Error while removing account " + accountIDtoRemove, exc);
                }
                break;
            }
        }
    ...
    private void handleSetAccount(Intent intent) {
        ...
        if (index == -1) {
            ...
            try {
                ...
                TyLog.e("Error while adding " + data.getIdUri(), exc);
            }
        } else {
            TyLog.d("Reconfiguring " + data.getIdUri());
            ...
            try {
                ...
                TyLog.e("Error while reconfiguring " + data.getIdUri(), exc);
            }
        }
    }
    ...
    private void loadNativeLibraries() {
        try {
            ...
            TyLog.d("OpenH264 loaded");
        } catch (UnsatisfiedLinkError error) {
            TyLog.e("Error while loading OpenH264 native library:"+error);
        ...
        try {
            ...
            TyLog.d("PJSIP pjsua2 loaded");
        } catch (UnsatisfiedLinkError error) {
            TyLog.e("Error while loading PJSIP pjsua2 native library:"+error);
            throw new RuntimeException(error);
        }
    }
    ...
    private void startStack() {
        ...
        try {
            ...
            if (codecPriorities != null) {
                TyLog.d("Setting saved codec priorities...");
                for (CodecPriority codecPriority : codecPriorities) {
                    TyLog.d("Setting " + codecPriority.getCodecId() + " priority to " + codecPriority.getPriority());
                    mEndpoint.codecSetPriority(codecPriority.getCodecId(), (short) codecPriority.getPriority());
                }
                TyLog.d("Saved codec priorities set!");
            } else {
            ...
            TyLog.d("PJSIP started!");
            mStarted = true;
            mBroadcastEmitter.stackStatus(true);

        } catch (Exception exc) {
            TyLog.e("Error while starting PJSIP:"+exc);
            mStarted = false;
        }
    }
    ...
    private void stopStack() {
        ...
        try {
            ...
            TyLog.d("PJSIP stopped");
            mBroadcastEmitter.stackStatus(false);

        } catch (Exception exc) {
            TyLog.e("Error while stopping PJSIP:"+exc);
    ...
    private ArrayList<CodecPriority> getCodecPriorityList() {
        ...
        if (!mStarted) {
            TyLog.e("Can't get codec priority list! The SIP Stack has not been " +
        ...
        try {
            ...
            TyLog.e("Error while getting codec priority list!:"+exc);
            return null;
        }
    }
    ...
    private void handleSetCodecPriorities(Intent intent) {
        ...
        try {
            ...
            TyLog.d(log.toString());
            mBroadcastEmitter.codecPrioritiesSetStatus(true);

        } catch (Exception exc) {
            TyLog.e("Error while setting codec priorities:"+exc);
            mBroadcastEmitter.codecPrioritiesSetStatus(false);
        }
    }
    ...
    private void handleGetRegistrationStatus(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while getting registration status for " + accountID, exc);
        }
    }
        ...
        if (!mActiveSipAccounts.isEmpty()) {
            for (String accountID : mActiveSipAccounts.keySet()) {
                try {
                    ...
                    TyLog.e("Error while removing " + accountID);
                }
            }
        }
    }
        ...
        if (!mConfiguredAccounts.isEmpty()) {
            for (SipAccountData accountData : mConfiguredAccounts) {
                try {
                    ...
                    TyLog.e("Error while adding " + accountData.getIdUri());
                }
            }
        }
    }
    ...
    private void addAccount(SipAccountData account) throws Exception {
        ...
        if (sipAccount == null || !sipAccount.isValid() || !account.equals(sipAccount.getData())) {
            ...
            TyLog.d("SIP account " + account.getIdUri() + " successfully added");
        } else {
            sipAccount.setRegistration(true);
        }
    }
    ...
    private void removeAccount(String accountID) {
        ...
        if (account == null) {
            TyLog.e("No account for ID: " + accountID);
            return;
        }

        TyLog.d("Removing SIP account " + accountID);
        account.delete();
        TyLog.d("SIP account " + accountID + " successfully removed");
    }
    ...
    void setSelfVideoOrientation(SipCall sipCall, int orientation) {
        try {
            ...
            TyLog.e("Error while changing video orientation");
        }
    }
    ...
    private void handleSwitchVideoCaptureDevice(Intent intent) {
        ...
        try {
            ...
            TyLog.e("Error while switching capture device:"+ex);
        }
    }
    ...
    private void handleMakeDirectCall(Intent intent) {
        ...
        TyLog.d("Making call to " + uri.getUserInfo());
        ...
        try {
            ...
            if (call != null) {
                ...
                TyLog.e("Error while making a direct call as Guest");
                mBroadcastEmitter.outgoingCall(accountID, -1, uri.getUserInfo(), false, false);
            }
        } catch (Exception ex) {
            TyLog.e("Error while making a direct call as Guest:"+ex);
            mBroadcastEmitter.outgoingCall(accountID, -1, uri.getUserInfo(), false, false);
        }
    }
